 ...
 public class DateUtil {
     ...
     public static Date createDate(Integer year, Integer month, Integer weekday, Integer count) {
         ...
         while (true) {
             ...
             if (count == -1 || !currentMonth.equals(month)) {
                 break;
             }
     ...
     public static int diffBusinessDays(Date begin, Date end, Set<String> holidaySet) {
         ...
         for (int i = 0; i < diffDays; ++i) {
             ...
             if (Boolean.TRUE.equals(isWeekend(date)) || holidaySet.contains(dateToStr(date))) {
                 continue;
             }
         ...
         for (int i = 0; i < dataArray.size(); i++) {
             ...
             for (String key : keys) {
                 if (dataJson.containsKey(key)) {
                     dataJson.replace(key, timeStampToStr(dataJson.getLong(key), pattern));
                 }
             }
         }
